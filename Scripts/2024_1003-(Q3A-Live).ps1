<#
   ...
   //¯¯\\__//¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯\\___    
   \\__//¯¯¯ Q3A-Live [~] Map Development Kit                                                               ___//¯¯\\   
    ¯¯¯\\__________________________________________________________________________________________________//¯¯\\__//   
        ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯    ¯¯¯¯    

    [Introduction]: Over the last month and a half, I've been building this utility, [Map Development Kit], which
    is a graphical user interface that integrates with the code-behind that I wrote to accelerate my ability to
    develop maps for (Quake III Arena/Quake Live). I am going to demonstrate how to use this application in order
    to produce a new map called "Dead Center".

    I have developed dozens of graphical user interfaces that rely on the (Windows Presentation Framework/WPF) by
    combining (Extensible Application Markup Language/XAML) and PowerShell. Most of them are included in the
    module FightingEntropy, but some offshoot GUI related stuff that doesn't require some of the elements in it
    aren't included the module.

    When I go about building a graphical user interface, I take into serious consideration how the application
    flows as it is being used, as it is essentially a document. Sometimes a complicated idea turns out to be
    pretty simple to build, but the inverse of that is true as well... Sometimes a very simple idea turns out
    to be incredibly complicated to build.

    This (function/application/program) as it *currently* stands, does require FightingEntropy, as it makes use of
    the console logging objects as well as some of the graphics like the window icon and the (file + folder)
    validation icons. You can find out how to install it at [https://github.com/mcc85s/FightingEntropy]

    As for (WPF/XAML/PowerShell), I like to use this trifecta because it reminds me of the many ways I went about
    building websites using (HTML/CSS/JavaScript) to design (print + web) documents as a graphic design artist.
    
    However, I believe that using PowerShell the way I do is rather phenomenal, and I wouldn't have gone about
    doing so if it weren't for a couple of dudes... 
    (Damien Van Robaeys/MVP), and (cybercommando Michael T. Neihaus/Microsoft).

    I won't get into what prompted me to really start programming, but the work those two did definitely pushed me
    to get serious about it, in addition to [Johan Arwidmark], [Mykael Nystrom], and [Paul Allen].

    As for "Dead Center", this isn't a map that I expect [Quake Live] players to take seriously, and one map it
    somewhat plays like is "Eye to Eye", which is (1) room with some added details and lava, and is really meant
    for just screwing around, and aim practice.

    I first created (testmap3/Dead Center) in July 2023 as I was reacquainting myself with GtkRadiant in this video:
    ====================================================================
    | Name                   | Duration | Url                          |
    | 2023_0717-(GtkRadiant) | 06:05:05 | https://youtu.be/-tGdz6oxXZI |
    ====================================================================

    I don't expect everybody to wanna watch a 6 hour video with no sound, but in that video I covered the basics
    of how to start from scratch in building a map, and how to add a bunch of details, and some brushwork. After
    spending the last couple of months going gung-ho fiddling around in the editor, all of my map editing skills
    have resurfaced... not unlike riding a bike.
#>

<#
    Uploading & Using Steam Workshop for QuakeLive
    start https://steamcommunity.com/sharedfiles/filedetails/?id=539821860
#>
...
<# 
    [Todo]:
    - Map details panel, find way to extract author, name, etc. from (*.map) file, then populate those details
    - Add control to (set/clear) credential, have set credential automatically change the tab to workshop
#>
