...
Try
{
    ...
    $AssemblyNames = 'mscorlib','System','System.Core','Microsoft.CSharp','WindowsBase','PresentationCore','PresentationFramework'
...
Class Q3ALiveByteSize
{
    ...
    {
        $This.Name   = $Name
        $This.Bytes  = @($Bytes;0)[[UInt64]$Bytes -eq 1]
        $This.GetUnit()
        $This.GetSize()
    }
    Q3ALiveByteSize([String]$Name,[String]$String)
    {
        $This.Name   = $Name
        $This.Convert($String)
    }
    ...
    {
        $Value = [Regex]::Matches($String,"^([0-9]*\.?[0-9]+)\s*(B|KB|MB|GB|TB)$")
        $U     = $Null
        $V     = $Null

        If ($Value.Groups.Count -eq 3)
        {
            Switch -Regex ($Value.Groups[2].Value)
            {
                "^B$"  { $U =     "Byte"; $V = 1   } 
                "^KB$" { $U = "Kilobyte"; $V = 1KB }
                "^MB$" { $U = "Megabyte"; $V = 1MB }
            ...
            $This.Unit  = $U
            $This.Bytes = [Double]($Value.Groups[1].Value) * $V
            $This.GetSize()
        }
    }
    GetUnit()
    {
        ...
        {
            { $_ -ge         0 -and $_ -le          870 } {     "Byte" }
            { $_ -ge       871 -and $_ -le       891289 } { "Kilobyte" }
            { $_ -ge    891289 -and $_ -le    912680550 } { "Megabyte" }
            { $_ -ge 912680550 -and $_ -le 934584883609 } { "Gigabyte" }
            { $_ -ge 934584883609                       } { "Terabyte" }
        }
    }
    GetSize()
    {
...
Class Q3ALiveFileSystemObject
{
    ...
    [String] $Fullname
    [Object] $Size
    Q3ALiveFileSystemObject([Object]$File)
    {
        $This.Modified  = $File.LastWriteTime.ToString("MM/dd/yyyy HH:mm:ss")
        $This.Type      = Switch -Regex ($File.GetType().Name) { DirectoryInfo { "Directory" } FileInfo { "File" } }
        $This.Name      = $File.Name
        $This.Extension = $File.Extension -Replace "\.", ""
        $This.Fullname  = $File.Fullname
        $This.Size      = $This.Q3ALiveByteSize($File.Length)
    }
    [Object] Q3ALiveByteSize([UInt64]$Bytes)
    {
        Return [Q3ALiveByteSize]::New($This.Type,$Bytes)
    }
}

Class Q3ALivePackageEntry
{
    [UInt32]       $Index
    [String]      $Source
    [String]        $Type
    ...
    [String]    $Fullname
    [Object]        $Size
    Q3ALivePackageEntry([UInt32]$Index,[String]$Source,[Object]$Entry)
    {
        $This.Index     = $Index
        $This.Source    = $Source
        $This.Type      = @("Directory","File")[$Entry.Length -ne 0]
        $This.Modified  = $Entry.LastWriteTime.ToString("MM/dd/yyyy HH:mm:ss")
        $This.Name      = $Entry.Name
        $This.Extension = If ($This.Type -eq "File") { $Entry.Name.Split(".")[-1] }
...
Class Q3ALiveComponentPackage
{
    ...
    [Object] $FileStream
    [Object] $Archive
    Q3ALiveComponentPackage([UInt32]$Index,[String]$Type,[Object]$Object)
    {
        $This.Index    = $Index
        $This.Type     = $Type
    ...
    Open()
    {
        ...
        Switch ($This.Exists)
        {
            0 { $This.FileStream = [System.IO.File]::Open($This.Fullname,'Create') }
            1 { $This.FileStream = [System.IO.File]::Open($This.Fullname,'Open')   }
        }

        $This.Exists     = [System.IO.File]::Exists($This.Fullname)
        $This.Archive    = [System.IO.Compression.ZipArchive]::New($This.FileStream,[System.IO.Compression.ZipArchiveMode]::Update)
    }
    Close()
    {
        If ($This.Archive)
    ...
    Refresh()
    {
        ...
        $Hash = @{ }
        ForEach ($Entry in $This.Archive.Entries)
        {
            $Hash.Add($Hash.Count,$This.Q3ALivePackageEntry($Hash.Count,$Entry))
        }

        $This.Output = $Hash[0..($Hash.Count-1)]
        $This.Close()
    }
    [Object] Q3ALivePackageEntry([UInt32]$Index,[Object]$Entry)
    {
        Return [Q3ALivePackageEntry]::New($Index,$This.Fullname,$Entry)
    }
    [Object] Q3ALiveByteSize([UInt64]$Bytes)
    {
...
Class Q3ALiveComponentBase
{
    ...
    Refresh()
    {
        ...
        If ($This.Exists)
        {
            ...
            $List    = $Tree.Output | ? Extension -eq pk3

            $Hash = @{ }
            ForEach ($Item in $List)
            {
                $Hash.Add($Hash.Count,$This.Q3ALiveComponentPackage($Hash.Count,"Component",$Item))
            }
    ...
    [Object] Q3ALiveComponentPackage([UInt32]$Index,[String]$Type,[Object]$Object)
    {
        Return [Q3ALiveComponentPackage]::New($Index,$Type,$Object)
    }
    [String] ToString()
    {
        Return $This.Fullname
...
Class Q3ALiveComponentItem
{
    ...
    {
        Return [Q3ALiveComponentBase]::New()
    }
    [String] Engine()
    {
...
Class Q3ALiveController
{
    ...
    GetSearch()
    {
        $This.Update(0,"Loaded [~] Search (Buffer/Timer)")
        $This.Search   = $This.Q3ALiveSearchObject()
    }
    ...
    {
        Return [Q3ALiveFileSystemObject]::New($File)
    }
    [Object] Q3ALiveXaml()
    {
