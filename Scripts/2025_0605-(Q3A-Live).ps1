...
If (!(Get-Module -Name FightingEntropy -EA 0 | ? Version -eq 2024.1.0))
{
    ...
    $Content = Invoke-RestMethod ("{0}?raw=true" -f $InsFtall.TrimEnd("`n"))
...
Class Q3ALiveRegistryRoot
{
    ...
    Check()
    {
        ...
        Switch ($Children.Count)
        {
            0
            {
                $Name = [DateTime]::Now.ToString("yyyyMMss_HHmmss")
                New-Item "$($This.Path)\$Name"
            }
            Default
            {
...
Class Q3ALiveMaster
{
    ...
    UpdateShaderlist()
    {
        ...
        $Shaderlist = "{0}\scripts\shaderlist.txt" -f $Ctrl.Q3A().Base
    ...
    SetLevelshot([String]$Path)
    {
        ...
        $FilePath = $Ctrl.Dependency.Get("Image").FilePath

        If (![System.IO.File]::Exists($Path))
        {
            $Ctrl.Update(1,"Exception [!] Invalid path `"$Path`"")
            Return
        }
    
        $Extension  = $Path.Split(".")[-1]

        If ($Extension -match "(tga|png)")
        {
            $Ctrl.Update(0,"Convert [~] $Path -> (*.jpg)")

            $NewPath = $Path.Replace($Extension,"jpg")
            $Param   = "convert `"$Path`" `"$NewPath`""
            Start-Process -FilePath $Filepath -ArgumentList $Param -NoNewWindow -Wait
            
            If (Test-Path $NewPath)
            {
                $Path = $NewPath
                $Ctrl.Update(0,"Converted [+] $Path")
            }
            Else
            {
                $Ctrl.Update(1,"Exception [!] Levelshot conversion failed")
            }
        }
    
        [System.IO.File]::Copy($Path,$Ctrl.Map.Levelshot.Path,$True)
        $Ctrl.Map.Levelshot.Check()
    
        If (!$Ctrl.Map.Levelshot.Exists)
        {
            $Ctrl.Update(1,"Exception [!] Levelshot not detected")
        }
        Else
        {
            $Ctrl.Update(0,"Updated [+] Levelshot: [$($Ctrl.Map.Levelshot.Path)]")

            $Preview = $Ctrl.Map.Levelshot.Path.Replace("levelshots\","levelshots\preview\")
            $Parent  = Split-Path $Preview -Parent
            ...
            $Param = "convert `"$($Ctrl.Map.Levelshot.Path)`" -resize 256x256! `"$Preview`""
    
            Start-Process -FilePath $FilePath -ArgumentList $Param -NoNewWindow -Wait
    
            Switch ([UInt32][System.IO.File]::Exists($Preview))
            {
                0
                {
                    $Ctrl.Update(1,"Exception [!] Unable to create preview image")
                }
                1
                {
                    $Ctrl.Update(0,"Complete [+] Preview: [$Preview]")
                }
            }
        }
    }
    ...
    Decompile([String]$Bsp)
    {
        ...
        <# [Implement BSPConv to detect version of bsp]#
        $FilePath = "C:\Users\mcadmin\Downloads\bspconv\bspconv.exe"

        bspconv.exe 46 pak00.pk3 --bsp:bloodrun.bsp
        $Param    = "46 `"{0}`"" -f $Bsp
        start-process -Filepath $Filepath -WorkingDirectory $Ctrl.Q3A().Base -Argumentlist $Param -NoNewWindow -Wait

        $Bsp = "C:\Program Files (x86)\Quake III Arena\baseq3\rtcq-old_IBSP46.bsp"
        #>
    ...
    MapApply()
    {
        ...
        $This.Xaml.IO.MapApply.IsEnabled           = 0
        ...
        $This.Xaml.IO.MapPlayText.Text                 = "+devmap {0} ffa" -f $This.Map.Name
    ...
    MapRefresh()
    {
        ...
        $This.Update(0,"Resetting [~] Map: $($This.Map.Name)")
    }
    MapSlot()
    {
        $Ctrl = $This
    ...
    MapLevelshotImport()
    {
        ...
        $Dialog.InitialDirectory = $This.ScreenshotPath()
        ...
        If ($Dialog.Filename)
        {
            If ($Dialog.Filename -eq $This.Map.Levelshot.Path)
            {
                $This.Update(0,"Exception [!] Cannot replace existing levelshot with itself")
            }
            Else
            {
    ...
    MapPlay()
    {
        ...
        {
            "^$" { "+devmap {0} ffa" -f $Ctrl.Map.Name } Default { $Ctrl.Xaml.IO.MapPlayText.Text }
        }
    ...
    SteamWorkshopPathApply()
    {
        ...
        $Ctrl.Xaml.IO.SteamWorkshopProjectUpdate.IsEnabled = 0
    ...
    SteamWorkshopProject()
    {
        ...
        $Leaf     = Split-Path $Ctrl.Map.Target -Leaf
        $File     = $Current.Content | ? Name -eq $Leaf

        $xMap     = Get-FileHash $Ctrl.Map.Target
        $Project  = Get-FileHash $File.Fullname
        
        $Ctrl.Xaml.IO.SteamWorkshopProjectUpdate.IsEnabled    = [UInt32]($xMap.Hash -ne $Project.Hash)
    ...
    SteamWorkshopProjectUpdate()
    {
        $Ctrl    = $This

        $Current  = $Ctrl.Steam.Current()
        $Ctrl.Update(0,"Updating [~] Project: [$($Current.Name)]")
        $xPath    = "{0}\content" -f $Current.Fullname

        # Map
        $Leaf     = Split-Path $Ctrl.Map.Target -Leaf
        [System.IO.File]::Copy($Ctrl.Map.Target,"$xPath\$Leaf",$True)

        # Preview
        $Leaf     = "preview.{0}" -f $Ctrl.Map.Levelshot.Name.Split(".")[-1]
        [System.IO.File]::Copy($Ctrl.Map.Levelshot.Path,"$xPath\$Leaf",$True)

        $Current.Populate()

        $Ctrl.Update(0,"Updated [+] Project: [$($Current.Name)]")
        $Ctrl.Xaml.IO.SteamWorkshopProjectUpdate.IsEnabled = 0
    }
    ...
    Stage([String]$Name)
    {
        ...
        Switch ($Name)
        {
            ...
            Map
            {
                ...
                $Ctrl.Xaml.IO.MapApply.Add_Click(
                {
                    $Ctrl.MapApply()
                })

                $Ctrl.Xaml.IO.MapClear.Add_Click(
                {
                    $Ctrl.MapClear()
                })
            ...
            Steam
            {
                ...
                $Ctrl.Xaml.IO.SteamWorkshopProjectUpdate.Add_Click(
                {
                    $Ctrl.SteamWorkshopProjectUpdate()
                })
    ...
    Initial([String]$Name)
    {
        ...
        Switch ($Name)
        {
            ...
            Map
            {
                ...
                $Ctrl.Xaml.IO.MapApply.IsEnabled               = 0
    ...
    {
        $xSteam = Get-ItemProperty $This.Registry.GetUninstallPath() | ? DisplayName -match Steam
        If (!$xSteam)
        {
        ...
        Else
        {
            Return $xSteam.UninstallString.Replace("uninstall.exe","userdata\1268058\760\remote\282440\screenshots")
        }
    }
    [String] ToString()
    {
...
<# 
    [Todo]:
    - Implement (*.tga) support to Xaml
    - Implement [screenshots] + [demos]
    - [Map panel]
      - Add to [shaderlist.txt] when [Shader] content is detected but is not specified, or even remove it
      - Any updates to map assets will refresh every panel and return to the last selected
      - Autocomplete for map name or use a datagrid showing the detected maps in the directory
      - [Refresh] button in addition to [Clear], maybe use a graphic for that instead of words
      - Checkbox to toggle (map/devmap)
      - Add combobox to select different rendering qualities (like in radiant)
      - Output panel should show the Fullname of every item, not the Name
      - Add a [Texture] subpanel
    - [Console panel]
      - Add a pause button for breaks

      ^ stuff I'll do after the map pack is ready for beta

    Screenshots: C:\Program Files (x86)\Steam\userdata\1268058\760\remote\282440\screenshots
#>
...
<#
      ...
      0  [crossfire]      : <current 06/05/25>
	  1  [breakthru]      : <current 06/05/25>
	  2  [ss1138]         : <current 06/05/25>
	  3  [suspanim]       : <current 06/05/25>
	  4  [deadcenter]     : <current 06/05/25>
	  5  [tranquilibrium] : <current 06/05/25>
	  6  [wisdom]         : <current 06/05/25>
	  7  [kerberos]       : <current 06/05/25>
	  8  [tempgrave]      : <current 06/05/25>
	  9  [rtcq]           : <current 06/05/25>
	  10 [insprod]        : <current 06/05/25>
	  11 [outofmyhead]    : <current 06/05/25>
	  12 [goliath]        : <current 06/05/25>

      BFG20K Map Pack (beta)
      https://steamcommunity.com/sharedfiles/filedetails/?id=3493925025
      
      File Size     365.927 MB
      Posted    Jun 5 @ 7:47pm
      Updated   Jun 6 @ 1:58pm
#>
